...
class CnnHeadPoseEstimator:
    ...
    def _allocate_yaw_variables(self):
        ...
        self.hy_conv1_weights = tf.Variable(
            tf.truncated_normal([3, 3, 3, 64], stddev=0.1)
        )
        ...
        self.hy_dense1_weights = tf.Variable(
            tf.truncated_normal([8 * 8 * 256, 256], stddev=0.1)
        )  # was [5*5*256, 1024]
        ...
        self.hy_out_weights = tf.Variable(
            tf.truncated_normal([256, self._num_labels], stddev=0.1)
        )
        ...
        def model(data):
            ...
            if DEBUG == True:
                print("SHAPE pool3: " + str(pool3.get_shape()))
            # Apply Normalization
            norm3 = tf.nn.lrn(pool3, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75)

            # Fully connected layer 4
            dense1 = tf.reshape(
                norm3, [-1, self.hy_dense1_weights.get_shape().as_list()[0]]
            )  # Reshape conv3
            if DEBUG == True:
    ...
    def load_yaw_variables(self, YawFilePath):
        ...
        if os.path.isfile(YawFilePath) == False:
            raise ValueError(
                "[DEEPGAZE] CnnHeadPoseEstimator(load_yaw_variables): the yaw file path is incorrect."
            )

        tf.train.Saver(
            (
                {
                    "conv1_yaw_w": self.hy_conv1_weights,
                    "conv1_yaw_b": self.hy_conv1_biases,
                    "conv2_yaw_w": self.hy_conv2_weights,
                    "conv2_yaw_b": self.hy_conv2_biases,
                    "conv3_yaw_w": self.hy_conv3_weights,
                    "conv3_yaw_b": self.hy_conv3_biases,
                    "dense1_yaw_w": self.hy_dense1_weights,
                    "dense1_yaw_b": self.hy_dense1_biases,
                    "out_yaw_w": self.hy_out_weights,
                    "out_yaw_b": self.hy_out_biases,
                }
            )
    ...
    def return_yaw(self, image, radians=False):
        ...
        if h == w and h == 64 and d == 3:
            ...
            if radians == True:
                return np.multiply(yaw_vector, np.pi / 180.0)  # to radians
            else:
                return yaw_vector
        ...
        if h == w and h > 64 and d == 3:
            ...
            feed_dict = {self.tf_yaw_input_vector: image_normalised}
            yaw_raw = self._sess.run([self.cnn_yaw_output], feed_dict=feed_dict)
            yaw_vector = np.multiply(
                yaw_raw, 100.0
            )  # cnn-out is in range [-1, +1] --> [-100, + 100]
            if radians == True:
                return np.multiply(yaw_vector, np.pi / 180.0)  # to radians
            else:
                return yaw_vector
        ...
        if h != w or w < 64 or h < 64:
            if h != w:
                raise ValueError(
                    "[DEEPGAZE] CnnHeadPoseEstimator(return_yaw): the image given as input has wrong shape. Height must equal Width. Height=%d,Width=%d"
                    % (h, w)
                )
            else:
                raise ValueError(
                    "[DEEPGAZE] CnnHeadPoseEstimator(return_yaw): the image given as input has wrong shape. Height and Width must be >= 64 pixel"
                )
        # wrong number of channels
        if d != 3:
            raise ValueError(
                "[DEEPGAZE] CnnHeadPoseEstimator(return_yaw): the image given as input does not have 3 channels, this function accepts only colour images."
            )
    ...
    def _allocate_pitch_variables(self):
        ...
        def model(data):
            ...
            # Fully connected layer 4
            dense1 = tf.reshape(
                norm3, [-1, self.hp_dense1_weights.get_shape().as_list()[0]]
            )  # Reshape conv3
            if DEBUG == True:
                print("SHAPE dense1: " + str(dense1.get_shape()))
            dense1 = tf.tanh(
    ...
    def _allocate_roll_variables(self):
        ...
        # [ 5*5 * previous_layer_out , num_hidden] wd1
        # here 5*5 is the size of the image after pool reduction (divide by half 3 times)
        self.hr_dense1_weights = tf.Variable(
            tf.truncated_normal([8 * 8 * 256, 256], stddev=0.1)
        )  # was [5*5*256, 1024]
        ...
        # Output layer
        self.hr_out_weights = tf.Variable(
            tf.truncated_normal([256, self._num_labels], stddev=0.1)
        )
        ...
        def model(data):
            ...
            conv1 = tf.tanh(
                tf.nn.bias_add(
                    tf.nn.conv2d(
                        X, self.hr_conv1_weights, strides=[1, 1, 1, 1], padding="SAME"
                    ),
            ...
            conv2 = tf.tanh(
                tf.nn.bias_add(
                    tf.nn.conv2d(
                        norm1,
                        self.hr_conv2_weights,
                        strides=[1, 1, 1, 1],
                        padding="SAME",
                    ),
                    self.hr_conv2_biases,
                )
            )
            if DEBUG == True:
                print("SHAPE conv2: " + str(conv2.get_shape()))
            ...
            conv3 = tf.tanh(
                tf.nn.bias_add(
                    tf.nn.conv2d(
                        norm2,
                        self.hr_conv3_weights,
                        strides=[1, 1, 1, 1],
                        padding="SAME",
                    ),
                    self.hr_conv3_biases,
                )
            )
            if DEBUG == True:
                print("SHAPE conv3: " + str(conv3.get_shape()))
            ...
            # Fully connected layer 4
            dense1 = tf.reshape(
                norm3, [-1, self.hr_dense1_weights.get_shape().as_list()[0]]
            )  # Reshape conv3
            if DEBUG == True:
                print("SHAPE dense1: " + str(dense1.get_shape()))
            dense1 = tf.tanh(
                tf.matmul(dense1, self.hr_dense1_weights) + self.hr_dense1_biases
            )
    ...
    def load_pitch_variables(self, pitchFilePath):
        ...
        if os.path.isfile(pitchFilePath) == False:
            raise ValueError(
        ...
        tf.train.Saver(
            (
                {
                    "conv1_pitch_w": self.hp_conv1_weights,
                    "conv1_pitch_b": self.hp_conv1_biases,
                    "conv2_pitch_w": self.hp_conv2_weights,
                    "conv2_pitch_b": self.hp_conv2_biases,
                    "conv3_pitch_w": self.hp_conv3_weights,
                    "conv3_pitch_b": self.hp_conv3_biases,
                    "dense1_pitch_w": self.hp_dense1_weights,
                    "dense1_pitch_b": self.hp_dense1_biases,
                    "out_pitch_w": self.hp_out_weights,
                    "out_pitch_b": self.hp_out_biases,
                }
            )
    ...
    def load_roll_variables(self, rollFilePath):
        ...
        if os.path.isfile(rollFilePath) == False:
            raise ValueError(
        ...
        tf.train.Saver(
            (
                {
                    "conv1_roll_w": self.hr_conv1_weights,
                    "conv1_roll_b": self.hr_conv1_biases,
                    "conv2_roll_w": self.hr_conv2_weights,
                    "conv2_roll_b": self.hr_conv2_biases,
                    "conv3_roll_w": self.hr_conv3_weights,
                    "conv3_roll_b": self.hr_conv3_biases,
                    "dense1_roll_w": self.hr_dense1_weights,
                    "dense1_roll_b": self.hr_dense1_biases,
                    "out_roll_w": self.hr_out_weights,
                    "out_roll_b": self.hr_out_biases,
                }
            )
    ...
    def return_pitch(self, image, radians=False):
        ...
        if h == w and h == 64 and d == 3:
            ...
            if radians == True:
                return np.multiply(pitch_vector, np.pi / 180.0)  # to radians
            else:
                return pitch_vector
        # If the image is > 64 pixel then resize it
        if h == w and h > 64 and d == 3:
            ...
            feed_dict = {self.tf_pitch_input_vector: image_normalised}
            pitch_raw = self._sess.run([self.cnn_pitch_output], feed_dict=feed_dict)
            pitch_vector = np.multiply(
                pitch_raw, 45.0
            )  # cnn-out is in range [-1, +1] --> [-45, + 45]
            if radians == True:
                return np.multiply(pitch_vector, np.pi / 180.0)  # to radians
            else:
                return pitch_vector
        ...
        if h != w or w < 64 or h < 64:
            raise ValueError(
                "[DEEPGAZE] CnnHeadPoseEstimator(return_pitch): the image given as input has wrong shape. Height and Width must be >= 64 pixel"
            )
        # wrong number of channels
        if d != 3:
            raise ValueError(
                "[DEEPGAZE] CnnHeadPoseEstimator(return_pitch): the image given as input does not have 3 channels, this function accepts only colour images."
            )
    ...
    def return_roll(self, image, radians=False):
        ...
        if h != w or w < 64 or h < 64:
            raise ValueError(
                "[DEEPGAZE] CnnHeadPoseEstimator(return_roll): the image given as input has wrong shape. Height and Width must be >= 64 pixel"
            )
        # wrong number of channels
        if d != 3:
            raise ValueError(
                "[DEEPGAZE] CnnHeadPoseEstimator(return_roll): the image given as input does not have 3 channels, this function accepts only colour images."
            )
...
class PnpHeadPoseEstimator:
    ...
    def __init__(self, cam_w, cam_h, dlib_shape_predictor_file_path):
        ...
        if IS_DLIB_INSTALLED == False:
            raise ValueError(
                "[DEEPGAZE] PnpHeadPoseEstimator: the dlib libray is not installed. Please install dlib if you want to use the PnpHeadPoseEstimator class."
            )
        if os.path.isfile(dlib_shape_predictor_file_path) == False:
            raise ValueError(
                "[DEEPGAZE] PnpHeadPoseEstimator: the files specified do not exist."
            )
        ...
        self.camera_matrix = np.float32(
            [[f_x, 0.0, c_x], [0.0, f_y, c_y], [0.0, 0.0, 1.0]]
        )
    ...
    def _return_landmarks(
        self, inputImg, roiX, roiY, roiW, roiH, points_to_return=range(0, 68)
    ):
    ...
    def return_roll_pitch_yaw(self, image, radians=False):
        ...
        retval, rvec, tvec = cv2.solvePnP(
            landmarks_3D, landmarks_2D, self.camera_matrix, self.camera_distortion
        )
