...
class HistogramColorClassifier:
    ...
    def addModelHistogram(self, model_frame, name=""):
        ...
        if self.hist_type == "HSV":
            model_frame = cv2.cvtColor(model_frame, cv2.COLOR_BGR2HSV)
        ...
        hist = cv2.calcHist(
            [model_frame], self.channels, None, self.hist_size, self.hist_range
        )
    ...
    def returnHistogramComparison(self, hist_1, hist_2, method="intersection"):
        ...
        if cv2.__version__.split(".")[0] == "3":
            if method == "intersection":
                ...
                raise ValueError(
                    "[DEEPGAZE] color_classification.py: the method specified "
                    + str(method)
                    + " is not supported."
                )
        else:
            if method == "intersection":
                ...
                comparison = cv2.compareHist(
                    hist_1, hist_2, cv2.cv.CV_COMP_BHATTACHARYYA
                )
            else:
                raise ValueError(
                    "[DEEPGAZE] color_classification.py: the method specified "
    ...
    def returnHistogramComparisonArray(self, image, method="intersection"):
        ...
        image_hist = cv2.calcHist(
            [image], self.channels, None, self.hist_size, self.hist_range
        )
        image_hist = cv2.normalize(image_hist, image_hist).flatten()
        counter = 0
        for model_hist in self.model_list:
            comparison_array[counter] = self.returnHistogramComparison(
                image_hist, model_hist, method=method
            )
            counter += 1
    ...
    def returnHistogramComparisonProbability(self, image, method="intersection"):
        ...
        @return a numpy array containg the comparison value between each pair image-model
        """
        comparison_array = self.returnHistogramComparisonArray(
