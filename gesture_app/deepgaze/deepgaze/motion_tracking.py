...
class ParticleFilter:
    ...
    def predict(self, x_velocity, y_velocity, std):
        ...
        self.particles[:, 0] += x_velocity + (
            np.random.randn(len(self.particles)) * std
        )  # predict the X coord
        self.particles[:, 1] += y_velocity + (
            np.random.randn(len(self.particles)) * std
        )  # predict the Y coord
    ...
    def update(self, x, y):
        ...
        self.weights += 1.0e-300  # avoid zeros
    ...
    def estimate(self):
        ...
        x_mean = np.average(self.particles[:, 0], weights=self.weights, axis=0).astype(
            int
        )
        y_mean = np.average(self.particles[:, 1], weights=self.weights, axis=0).astype(
            int
        )
    ...
    def resample(self, method="residual"):
        ...
        if method == "multinomal":
            ...
            indices = np.searchsorted(
                cumulative_sum, np.random.uniform(low=0.0, high=1.0, size=N)
            )
            ...
            raise ValueError(
                "[DEEPGAZE] motion_tracking.py: the resempling method selected '"
                + str(method)
                + "' is not implemented"
            )
    ...
    def drawParticles(self, frame, color=[0, 0, 255], radius=2):
        ...
        for x_particle, y_particle in self.particles.astype(int):
            cv2.circle(
                frame, (x_particle, y_particle), radius, color, -1
            )  # RED: Particles
