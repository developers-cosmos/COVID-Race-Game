...
class BackProjectionColorDetector:
    ...
    def returnFiltered(
        self, frame, morph_opening=True, blur=True, kernel_size=5, iterations=1
    ):
        ...
        frame_threshold = self.returnMask(
            frame,
    ...
    def returnMask(
        ...
        if morph_opening == True:
            ...
            frame_hsv = cv2.morphologyEx(
                frame_hsv, cv2.MORPH_OPEN, kernel, iterations=iterations
            )
...
class MultiBackProjectionColorDetector:
    ...
    def returnMask(
        ...
        for template_hsv in self.template_hsv_list:
            # Set the template histogram
            template_hist = cv2.calcHist(
                [template_hsv], [0, 1], None, [256, 256], [0, 256, 0, 256]
            )
            # Normalize the template histogram and apply backprojection
            cv2.normalize(template_hist, template_hist, 0, 255, cv2.NORM_MINMAX)
            frame_hsv_back = cv2.calcBackProject(
                [frame_hsv], [0, 1], template_hist, [0, 256, 0, 256], 1
            )
            # Get the kernel and apply a convolution
            kernel = cv2.getStructuringElement(
                cv2.MORPH_ELLIPSE, (kernel_size, kernel_size)
            ...
            if morph_opening == True:
                kernel = np.ones((kernel_size, kernel_size), np.uint8)
                frame_hsv_clean = cv2.morphologyEx(
                    frame_hsv_clean, cv2.MORPH_OPEN, kernel, iterations=iterations
                )
            # Applying Gaussian Blur
            if blur == True:
                frame_hsv_clean = cv2.GaussianBlur(
                    frame_hsv_clean, (kernel_size, kernel_size), 0
                )
            # Get the threshold
...
class RangeColorDetector:
    ...
    def returnFiltered(
        ...
        """
        frame_filtered = self.returnMask(
            frame,
            morph_opening=morph_opening,
            blur=blur,
            kernel_size=kernel_size,
    ...
    def returnMask(
        ...
        if morph_opening == True:
            kernel = np.ones((kernel_size, kernel_size), np.uint8)
            frame_filtered = cv2.morphologyEx(
                frame_filtered, cv2.MORPH_OPEN, kernel, iterations=iterations
            )
        # Applying Gaussian Blur
        if blur == True:
            frame_filtered = cv2.GaussianBlur(
                frame_filtered, (kernel_size, kernel_size), 0
            )
