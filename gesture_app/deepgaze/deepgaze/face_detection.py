...
class HaarFaceDetector:
    ...
    def __init__(self, frontalFacePath, profileFacePath):
        ...
        if (
            os.path.isfile(frontalFacePath) == False
            and os.path.isfile(profileFacePath) == False
        ):
            raise ValueError(
                "[DEEPGAZE] haarCascade: the files specified do not exist."
            )
    ...
    def returnFacePosition(
        ...
        order = list()
        if lastFaceType == 0 or lastFaceType == 1:
            order = (1, 2, 3, 4, 5)
        if lastFaceType == 2:
            order = (2, 1, 3, 4, 5)
        if lastFaceType == 3:
        ...
        for position in order:
            ...
            if runFrontalRotated == True and position == 2:
                ...
                M = cv2.getRotationMatrix2D(
                    (cols / 2, rows / 2), rotationAngleCCW, 1
                )  # 30 degrees ccw rotation
                inputImgRot = cv2.warpAffine(inputImg, M, (cols, rows))
                self._findFrontalFace(
            ...
            if runFrontalRotated == True and position == 3:
                rows, cols = numpy.shape(inputImg)
                M = cv2.getRotationMatrix2D(
                    (cols / 2, rows / 2), rotationAngleCW, 1
                )  # 30 degrees cw rotation
                inputImgRot = cv2.warpAffine(inputImg, M, (cols, rows))
                self._findFrontalFace(
                    inputImgRot, rotatedFrontalScaleFactor, minSizeX, minSizeY
                )
            ...
            if runRight == True and position == 5:
                ...
                self._findProfileFace(
                    flipped_inputImg, rightScaleFactor, minSizeX, minSizeY
                )
                if self.is_face_present == True:
                    self.face_type = 5
    ...
    def returnMultipleFacesPosition(
        self,
        inputImg,
        runFrontal=True,
        runFrontalRotated=True,
        ...
        # Cascade: frontal faces
        if runFrontal == True:
            faces = self._findMultipleFrontalFaces(
                inputImg, frontalScaleFactor, minSizeX, minSizeY
            )
            if self.is_face_present == True:
        ...
        # Cascade: left profiles
        if runLeft == True:
            faces = self._findMultipleProfileFaces(
                inputImg, leftScaleFactor, minSizeX, minSizeY
            )
            if self.is_face_present == True:
                allTheFaces = numpy.append(allTheFaces, faces, axis=0)
        # Cascade: right profiles
    ...
    def _findFrontalFace(
        self, inputImg, scaleFactor=1.1, minSizeX=30, minSizeY=30, minNeighbors=4
    ):
